...
export function StateEngine({ asset, analysis, memory, onMemoryChange }: { asset: Asset, analysis?: AssetAnalysis, memory: EngineMemory | null, onMemoryChange: (m: EngineMemory) => void }) {
  const clusters = useMemo(() => {
    if (analysis?.candles && analysis.candles.length >= 5) {
      return deriveClusterFromCandles(analysis.candles, asset.change24h);
    }
    return deriveClusters({ sparkline: asset.sparkline, change24h: asset.change24h });
  }, [analysis, asset]);
  ...
  useEffect(() => {
    ...
    const { snapshot, memory: newMemory } = evaluate(clusters, prior);
    saveMemory(asset.id, newMemory);
    setSnapshot(snapshot);
    onMemoryChange(newMemory);
  }, [asset.id, clusters, onMemoryChange]);
